/**
 * PONT D'API FRONTEND SWITCH FINTECH BÉNIN (CLIENT PWA -> API REST DÉCOUPLÉE)
 * Fichier : assets/js/api_bridge.js
 */

window.SwitchApi = {
  get baseUrl() {
    if (window.SWITCH_CONFIG && window.SWITCH_CONFIG.API_URL) {
      return window.SWITCH_CONFIG.API_URL.replace(/\/+$/, '');
    }
    return '/api/v1';
  },
